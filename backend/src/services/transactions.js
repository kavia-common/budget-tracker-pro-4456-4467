...
async function listTransactions(userId, filters) {
  ...
  if (filters.type) { where.push(`t.type = $${idx++}`); params.push(filters.type); }

  const whereSql = where.length ? `WHERE ${where.join(' AND ')}` : '';

  const listSql = `
    SELECT t.id, t.account_id, t.category_id, t.amount, t.type, t.description, t.date
    FROM transactions t
    ${whereSql}
...
async function createTransaction(userId, payload) {
  ...
  const { rows } = await query(
    `INSERT INTO transactions (user_id, account_id, category_id, amount, type, description, date)
     ...
     RETURNING id, account_id, category_id, amount, type, description, date`,
    [
      userId,
      payload.account_id,
